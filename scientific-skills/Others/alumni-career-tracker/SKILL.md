...
- Use this skill when the task is to Analyze laboratory alumni career trajectories and outcomes to provide data-driven.
...
This skill accepts requests that match the documented purpose of `alumni-career-tracker` and include enough context to complete the workflow safely.

Do not continue the workflow when the request is out of scope, missing a critical input, or would require unsupported assumptions. Instead respond:

> `alumni-career-tracker` only handles its documented workflow. Please provide the missing required inputs or switch to a more suitable skill.
